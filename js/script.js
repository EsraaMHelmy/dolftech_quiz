...
var startButton = document.getElementById("startButton"),
    ...
    // var images = ['images/egypt.png','images/saudia.png','images/tunisia.png'];


var allQuestions = [
	{
		question: "- ماهي عاصمة مصر",
		choices: ["القاهرة ", "دبي", "المنامة"],
		correctAnswer: 0,
		is_image: 0,
	},
	{
		question: "- ماهي عاصمة الامارات",
		choices: ["دبي", "الرياض", "ابو ظبي"],
		correctAnswer: 2,
		is_image: 0,
	},
	{
		question:  "- عاصمة تونس",
		choices: ["القاهرة", "تونس ", "الرياض"],
		correctAnswer: 1,
		is_image: 0,
	},
	{

		question: "- علم المملكة العربية السعودية",
		choices: ["egypt.png" , "tunisia.png"  ,"saudia.png" ],
		correctAnswer: 2,
		is_image: 1,
    },
    {
...
message.innerHTML = "مرحبا بكم في هذا الاختبار! <br /> هل تود أن تبدأ؟";
...
function showQuestion() {
	...
	for (var i = 0; i < choices.length; i++) {
		 ...
		 * in case image
		 */
		if(quizItem.is_image) {
			/**
			 * custom style to input to show it 
			 */
			input.style.opacity = 1;
			div.appendChild(input);
			var img = document.createElement("img");
			img.setAttribute("class", "flag_img");
			img.setAttribute("src", "./images/"+choices[i]);
...
function showProgress(index) {
	...
	progressMeterText.innerHTML = index + " من " + quizLength;
	if (index === 0) {
...
function showQuizButtons() {
	...
	if (index === quizLength) {
		...
		h2.innerHTML = "هل ترغب في رؤية درجاتك؟";
		form.appendChild(h2);
	} else {
...
function nextQuestion() {
	...
	if (storedAnswers[index] == null) {
		warning.innerHTML = "! الرجاء اختيار إجابة ";
		return;
	}
...
function showScore() {
	...
	for (var i = 0; i < storedAnswers.length; i++) {
		...
		output = output + "<p>السؤال " + (i + 1) + " : " + questionResult + "</p> ";
		totalScore += score;
	}
	...
	if (totalScore === allQuestions.length) {
		message.innerHTML =
			...
			" من " +
			allQuestions.length +
			"!<br />يمكنك أن تفعل الاختبار مرة أخرى ، على الرغم من أنك لا تحتاج حقا ";
	} else if (totalScore <= 1) {
		message.innerHTML =
			" يمكن انت تتدرب اكثر , لقد حصلت على" +
			totalScore +
			" من " +
			allQuestions.length +
			".<br />هل تريد أن تفعل الاختبار مرة أخرى ؟";
	} else {
		message.innerHTML =
			" حسنا هذا ليس سيئا ! لقد حصلت على" +
			totalScore +
			" من " +
			allQuestions.length +
			".<br />هل تريد أن تفعل الاختبار مرة أخرى ؟";
...
function showUserScores(e) {
	...
	if (userScores.length === 0) {
		string = "ليس لديك أي درجات حتى الآن";
	}
